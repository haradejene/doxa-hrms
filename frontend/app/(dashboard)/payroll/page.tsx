...
import {
  ChevronLeft, ChevronRight, Download, Printer, Play, RotateCcw,
  CheckCircle2, FileClock, Search, X, AlertCircle, Loader2, Users, Wallet, Receipt,
} from 'lucide-react'
import {
  getPayrollSheet, getPayrollPeriods, processPayroll,
  type PayrollSheet, type PayrollPeriod, type PayrollRecord, type PayrollTotals,
...
export default function PayrollPage() {
  ...
  useEffect(() => { loadSheet(period) }, [period, loadSheet])

  useEffect(() => {
    getPayrollPeriods().then(setPeriods).catch(() => setPeriods([]))
  }, [])

  const refreshPeriods = () => getPayrollPeriods().then(setPeriods).catch(() => {})
  ...
  const exportCsv = () => {
    ...
    const lines = [
      ...
      '',
      COLUMNS.map(c => esc(c.sub ? `${c.label} ${c.sub}` : c.label)).join(','),
      ...rows.map(r => COLUMNS.map(c => cell(c.value(r))).join(',')),
      COLUMNS.map(c => (c.key === 'no' ? esc('TOTAL') : c.total ? cell(c.total(totals)) : '')).join(','),
    ]
  ...
  return (
    <div className="space-y-5">
      ...
      <div data-no-print className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col xl:flex-row gap-3 xl:items-center">
        <div className="flex items-center gap-1">
          <button
            onClick={() => setPeriod(shiftMonth(period, -1))}
            className="h-10 w-10 grid place-items-center rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors"
          ...
          <select
            ...
            className="h-10 min-w-[15rem] rounded-xl border border-gray-200 bg-gray-50 px-3 text-sm font-semibold text-gray-900 focus:bg-white focus:border-violet-400 focus:ring-4 focus:ring-violet-100 outline-none transition-all"
          >
            {(periods.length ? periods : [{ period, label: period, status: 'draft' as const, ethiopian: null as any }]).map(p => (
              <option key={p.period} value={p.period}>
                {p.label}
                {p.ethiopian ? ` · ${p.ethiopian.month_name} ${p.ethiopian.year} E.C` : ''}
                {p.status === 'processed' ? ' ✓' : ''}
              </option>
            ))}
          </select>
        ...
        {sheet && (
          ...
          >
            {sheet.status === 'processed'
              ? <><CheckCircle2 className="h-3.5 w-3.5" /> Processed {shortDate(sheet.processed_at)}</>
              : <><FileClock className="h-3.5 w-3.5" /> Draft — not yet processed</>}
          </span>
        )}
      ...
      {!loading && sheet && (
        <>
          ...
          {search && (
            <p data-no-print className="text-xs text-gray-400 text-center">
